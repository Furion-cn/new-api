...
func getAndValidateTextRequest(c *gin.Context, relayInfo *relaycommon.RelayInfo) (*dto.GeneralOpenAIRequest, error) {
	...
	case relayconstant.RelayModeChatCompletions:
		if len(textRequest.Messages) == 0 && textRequest.Input == nil {
...
func TextHelper(c *gin.Context, relayInfo *relaycommon.RelayInfo, textRequest *dto.GeneralOpenAIRequest) (openaiErr *dto.OpenAIErrorWithStatusCode) {
	...
	defer func() {
		if funcErr != nil {
			metrics.IncrementRelayRequestFailedCounter(strconv.Itoa(relayInfo.ChannelId), relayInfo.ChannelName, relayInfo.ChannelTag, relayInfo.BaseUrl, textRequest.Model, relayInfo.Group, strconv.Itoa(funcErr.StatusCode), strconv.Itoa(relayInfo.UserId), relayInfo.UserName, funcErr.Error.Message, 1)
		} else {
	...
	if usage == nil {
		...
		funcErr = service.OpenAIErrorWrapperLocal(fmt.Errorf("usage is nil"), "usage_nil", http.StatusInternalServerError)
		return funcErr
	}

	// 将响应体写入 gin context，供 postConsumeQuota 使用
...
func postConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo,
	...
	// Record zero or negative token metrics
	if promptTokens <= 0 {
		metrics.IncrementPromptTokensZeroOrNegative(strconv.Itoa(relayInfo.ChannelId), relayInfo.ChannelName, modelName, relayInfo.Group, strconv.Itoa(relayInfo.UserId), userName, tokenName, 1)
	}
	if completionTokens <= 0 {
		metrics.IncrementCompletionTokensZeroOrNegative(strconv.Itoa(relayInfo.ChannelId), relayInfo.ChannelName, modelName, relayInfo.Group, strconv.Itoa(relayInfo.UserId), userName, tokenName, 1)
	}
	if thinkingTokens <= 0 {
		metrics.IncrementThinkingTokensZeroOrNegative(strconv.Itoa(relayInfo.ChannelId), relayInfo.ChannelName, modelName, relayInfo.Group, strconv.Itoa(relayInfo.UserId), userName, tokenName, 1)
	}
	if totalTokens <= 0 {
		metrics.IncrementTotalTokensZeroOrNegative(strconv.Itoa(relayInfo.ChannelId), relayInfo.ChannelName, modelName, relayInfo.Group, strconv.Itoa(relayInfo.UserId), userName, tokenName, 1)
	}
